<?php
	...
	wp_register_style('_views_settings', plugins_url('/_views_settings.css', __FILE__));
	wp_enqueue_style('_views_settings');
	
    wp_enqueue_script('_views_settings', plugins_url('/_views_settings.js', __FILE__),array('jquery'));

	/* Do not alter this file in any way. */
	function _rw_field_edit__views_settings($field){
		...
		foreach($wp_post_types as $slug=>$type){
			if(!in_array($slug, array('attachment','revision','nav_menu_item','content-type','views','view')))
...
//foreach ($taxonomies as $views ) {
//  echo '<p>'. $views. '</p>';
//}
		$arguments['options'] = (array) maybe_unserialize($arguments['options']);
		$options = array(
			array(0,'Hierarchical','hierarchical','Is this views hierarchical (have descendants) like categories or not hierarchical like tags'),
			array(1,'Public','public','Should this views be exposed in the admin UI'),
			array(1,'Show in Navigation','show_in_nav_menus','Make available for selection in navigation menus'),
			array(1,'Show User Interface','show_ui','Generate a default UI for managing this views'),
			array(1,'Tagcloud','show_tagcloud','Allow the Tag Cloud widget to use this views')
		);
	?>
	...
	<tbody class="label-slug">
		...
		<tr>
			...
			<td><input type="text" name="post_title" tabindex="<?php $i++; echo $i; ?>" value="<?php echo $post->post_title ?>" id="title">
				<p class="description">Name of views</p></td>
		</tr>		
		<tr>
			<th>Sigular Title</th>
	...
	<tbody>
		...
		<tr>
			...
			<td><input type="checkbox" name="arguments[content-types][]" tabindex="<?php $i++; echo $i; ?>" value="<?php echo $slug; ?>"<?php if(in_array($slug, $arguments['content-types'])) echo ' checked="checked"'; ?>/>
				<span class="description">Add views to <?php echo $title; ?></span></td>
		</tr>
		<?php endforeach; ?>
	</tbody>
	<tbody>
