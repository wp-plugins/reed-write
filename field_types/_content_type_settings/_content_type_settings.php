<?php
	...
	function field_type_edit__content_type_settings($field){
		...
		$arguments = array(
			...
			'menu_icon' => plugins_url('/icons/'.(isset($type['arguments']['menu_icon']) ? $type['arguments']['menu_icon'] : 'letter-'.$type['arguments']['slug'][0].'.png'), __FILE__),
...
<div id="content-type-settings">
	<table class="form-table">
		<tbody class="label-slug">
			...
			<tr>
				<th>Name</th>
				<td><input type="text" name="post_title" tabindex="<?php $i++; echo $i; ?>" value="<?php echo $post->post_title ?>" id="name" />
					<p class="description">Name of Post Type</p></td>
			</tr>
			<tr>
				<th>Sigular Name</th>
				<td><input type="text" name="arguments[name-singular]" tabindex="<?php $i++; echo $i; ?>" value="<?php echo $arguments['name-singular']; ?>" id="name-singular" />
					<p class="description">Singular Name of Post Type</p></td>
			</tr>
			<tr>
				<th>Slug</th>
		...
		<tbody>
			<tr class="title">
				<td colspan="2"><h2>Support Options</h2></td>
			</tr>
		...
		<tbody>
			<tr class="title">
				<td colspan="2"><h2>Taxonomy Types</h2></td>
			</tr>			
			...
			<tr>
				...
				<td>
					<select id="rewrite_type_select" name="arguments[rewrite_type]">
						<option<?php value_type('1', $arguments['rewrite_type'], 'selected'); ?>>Standard rewrite</option>
						<option<?php value_type('2', $arguments['rewrite_type'], 'selected'); ?>>Custom rewrite</option>
						<option<?php value_type('', $arguments['rewrite_type'], 'selected'); ?>>No rewrite</option>
					</select>
				</td>
			</tr>			
		</tbody>
		<tbody id="rewrite_options"<?php echo $arguments['rewrite_type'] != '2' ? ' style="display: none;"' : ''; ?>>
			...
			<tr>
				<th>With Front</th>
				<td><input type="checkbox" class="rewrite_with_front" name="arguments[rewrite_with_front]" <?php value_type('1', $arguments['rewrite_with_front'], 'checked'); ?>/><input class="front" type="hidden" value="<?php echo get_option('show_on_front'); ?>" /> <span class="description">Allow permalinks to be prepended with front base.</span></td>
			</tr>
<?php /*?>			<tr>
			...
			<tr class="title">
				<td colspan="2"><h2>Menu Options</h2></td>
			</tr>
			<tr>
				<th>Presentation</th>
				<td>
					<select id="show_menu_select" name="arguments[menu_type]">
						<option<?php value_type('1', $arguments['menu_type'], 'selected'); ?>>Display standard menu item</option>
						<option<?php value_type('2', $arguments['menu_type'], 'selected'); ?>>Display under More Content</option>
						<option<?php value_type('', $arguments['menu_type'], 'selected'); ?>>Do not display</option>
					</select>
				</td>
			</tr>
...
100 - below second separator" style="cursor: help;">?</a></p></td>
		...
		<tbody>
			<tr id="advanced-setting-option">
				<td colspan="2" style="text-align: center;"><a href="#">Show Advanced Options</a>
					<input type="hidden" value="<?php echo $arguments['advanced'] ? ($arguments['advanced']) : 'no'; ?>" name="arguments[advanced]" id="settings-advanced-show" /></td>
			</tr>
		</tbody>
		<tbody id="advanced-settings" style="display: none; ">
			<tr class="title">
